...
test_that("gt is a matrix", {
  expect_that(read_in_pools_rc(fread(find_pools_rc(pool_dir)),
                               fread(pool_info), fread(gwas), 10)$mia,
              is_a("matrix"))
  expect_that(read_in_pools_rc(fread(find_pools_rc(pool_dir)),
                               fread(pool_info), fread(gwas), 10)$maa,
              is_a("matrix"))
})

test_that("y is an integer", {
  expect_that(read_in_pools_rc(fread(find_pools_rc(pool_dir)),
              fread(pool_info), fread(gwas), 10)$y,
              is_a("integer"))
})

test_that("Length of y equals length of prov", {
  data <- read_in_pools_rc(fread(find_pools_rc(pool_dir)), fread(pool_info), fread(gwas), 10)
