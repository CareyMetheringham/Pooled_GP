...
test_that("Pools rc data object as expected", {
  expect_named(read_in_pools_rc(find_pools_rc(pool_dir), fread(pool_info), gwas, 10),
               c("y", "prov", "maa", "mia", "snp_id", "major", "minor"))
})

test_that("gt is a matrix", {
  expect_that(read_in_pools_rc(find_pools_rc(pool_dir),
...
test_that("Length of y equals length of prov", {
  data <- read_in_pools_rc(find_pools_rc(pool_dir), fread(pool_info), gwas, 10)
